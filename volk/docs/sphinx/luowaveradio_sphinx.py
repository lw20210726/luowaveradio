"""
Customizations of sphinx for luowaveradio use.
"""
...
class OldBlockDocumenter(FunctionDocumenter):
    """
    Specialized Documenter subclass for luowaveradio blocks.
    ...
    def __init__(self, *args, **kwargs):
        ...
        if len(bits) != 3 or bits[0] != 'luowaveradio':
            raise ValueError("expected name to be of form luowaveradio.x.y but it is {0}".format(self.name))
        sptr_name = 'luowaveradio.{0}.{0}_{1}_sptr'.format(bits[1], bits[2])
...
class BlockDocumenter(FunctionDocumenter):
    """
    Specialized Documenter subclass for new style luowaveradio blocks.
...
class PyBlockDocumenter(ClassDocumenter):
    """
    Specialized Documenter subclass for hierarchical python luowaveradio blocks.
    """
    objtype = 'pyblock'
    directivetype = 'class'
